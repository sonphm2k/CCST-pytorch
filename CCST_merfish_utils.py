...
def draw_map_merfish(args, adj_0, draw_constructed = 1, draw_nerighbor=1):
    df = pd.read_csv(args.result_path+ 'types.txt', header=None, index_col=None, delimiter='\t')
    df_np = df.to_numpy()
    cell_cluster_type_list = df_np[:,2] #
    n_clusters = max(cell_cluster_type_list) + 1 # start from 0

    import openpyxl
    workbook = openpyxl.load_workbook('dataset/MERFISH/pnas.1912459116.sd15.xlsx') # No arg.dataset_path
    # Color default
    if n_clusters <= 5:
        from matplotlib.colors import ListedColormap
        clusters_name = ['C0', 'C1', 'C2', 'C3', 'C4']
        colors_list = ['darkblue', 'orange', 'yellow', 'green', 'crimson']
    else:
    ...
    for sheet_name in ['Batch 1', 'Batch 2', 'Batch 3']:
        ...
        if n_clusters <= 5:
            sc_cluster = plt.scatter(x=sheet_X, y=sheet_Y, s=25, c=cell_cluster_batch_list, cmap=ListedColormap(colors_list))
        ...
        if draw_constructed:
            ...
            point = []
            for i in zip(sheet_X, sheet_Y):
                point.append(list(i))
            point = np.array(point)
            # distance all point
            distance = pdist(point)
            idx = np.vstack(np.triu_indices(len(point), k=1)).T
            ## idx for distance < threshold
            conditional = idx[distance <= 200]
            idx = list(set(conditional.flatten()))
            # print(idx)
            x_points = sheet_X[idx]
            y_points = sheet_Y[idx]
            segs = point[conditional]
            constructed_cell_cluster_batch_list = np.array(cell_cluster_batch_list)[idx]
            # Plot
            fig, ax = plt.subplots()
            ax.add_collection(LineCollection(segs, linewidths=(1), color='gray'))
            sc_cluster = plt.scatter(x=x_points, y=y_points, s=20, c=constructed_cell_cluster_batch_list, cmap=ListedColormap(colors_list), zorder = 2.5)
            ax.legend(handles=sc_cluster.legend_elements()[0], labels=clusters_name, bbox_to_anchor=(1, 0.8),loc='center left')
            plt.savefig(args.result_path + '/constructed_cluster_' + sheet_name.replace(' ', '') + '.png', dpi=400, bbox_inches='tight')
            plt.clf()
    # draw barplot
    if draw_nerighbor:
        ...
        for clusters_i in range(n_clusters):
            # if clusters_i >= 2:
            #     clusters_i += 1
            plt.bar(range(n_clusters), norm_barplot[clusters_i], label='graph '+ str(clusters_i), color = colors_list)
            plt.xlabel('Cluster')
            plt.ylabel('Neighbour enrichment ratio')
...
def CCST_on_MERFISH(args):
    ...
    adj_0, adj, X_data, cell_batch_info = get_MERFISH_data(args)
