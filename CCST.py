...
def train_DGI(args, data_loader, in_channels):
    ...
    if args.load:
        ...
        # DGI_model.load_state_dict(torch.load(DGI_filename)) # GPU
        DGI_model.load_state_dict(torch.load(DGI_filename, map_location=device)) # CPU
...
def Umap(args, X, label, n_clusters, score):
    ...
    if score:
        plt.text(0.0, 0.0, plt.title('UMAP projection'))
    if score:
